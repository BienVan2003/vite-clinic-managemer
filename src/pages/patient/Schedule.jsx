...
import { apiAppointmentPatientWeek, apiShiftList } from '../../services'
...
export default function Schedule() {
    ...
    const fetchSchedule = async () => {
        ...
        const res = await apiAppointmentPatientWeek({
            patientId: user.id,
    ...
    return (
        <div className='flex flex-col'>
            ...
            <div className="w-full flex p-6 mt-6">
                <div className='border-[1px] w-[60px] text-sm bg-green-700 text-white'>
                ...
                </div>
                {
                    dayOfWeeks.map(item => {
                        return <div key={item.date} className=''>
                            <div className='px-1 h-[32px] font-semibold text-sm flex items-center justify-center bg-green-700 text-white'>{item.day} ({formatddMMyyyy(item.date)})</div>
                            {
                                shifts.map(s => {
                                    const check = schedule?.find(st => st.shiftSchedule.id === s.id && formatyyyyMMdd(st.dateSchedule) === item.date && st.patient.id == user.id)
                                    if (
                                        check
                                    ) {
                                        return (
